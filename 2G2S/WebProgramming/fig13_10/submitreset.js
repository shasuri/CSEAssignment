   ...
   myForm.addEventListener( "submit", 
      ...
      {                                                         
         if(confirm( "Are you sure you want to submit?" ))
         {
            ...
            var myObj = {"name": name, "email": email, "like": like, "comments": comments};
            sessionStorage.setItem(name, JSON.stringify(myObj));
            loadData();
            return true;
         }
         else
...
function loadData() {
   ...
   if(sessionStorage.length > 0)
   {
      ...
      for (var i = 0; i < length; ++i) 
      {
         ...
         var obj = JSON.parse(sessionStorage.getItem(keys[i]));
         res += "<li>" + obj.name + ", " + obj.email + ", " + obj.like + ", " + obj.comments + "</li>";
      }
      res += "</ul>";
      document.getElementById("savedData").innerHTML = res;
   }
}
