...
void DES_CBC_Enc(BYTE *p_text, BYTE *c_text, BYTE *IV, BYTE *key, int msg_len)
{
    int xor_index, enc_index, pre_xor_index;
    int index = (msg_len % BLOCK_SIZE) ? (msg_len / BLOCK_SIZE + 1) : (msg_len / BLOCK_SIZE);

    for (int j = 0; j < BLOCK_SIZE; j++)
    {
        p_text[j] = p_text[j] ^ IV[j];
    }
    ...
    for (int i = 1; i < index; i++)
    {
        for (int j = 0; j < BLOCK_SIZE; j++)
        {
            xor_index = i * BLOCK_SIZE + j;
            pre_xor_index = (i - 1) * BLOCK_SIZE + j;
            p_text[xor_index] = p_text[xor_index] ^ c_text[pre_xor_index];
        }

        enc_index = i * BLOCK_SIZE;
        DES_Encryption(p_text + enc_index, c_text + enc_index, key);
    }
}
...
void DES_CBC_Dec(BYTE *c_text, BYTE *d_text, BYTE *IV, BYTE *key, int msg_len)
{
    ...
    for (int j = 0; j < BLOCK_SIZE; j++)
    {
        d_text[j] = d_text[j] ^ IV[j];
    }

    for (int i = 1; i < index; i++)
    {
        enc_index = i * BLOCK_SIZE;
        DES_Decryption(c_text + enc_index, d_text + enc_index, key);

        for (int j = 0; j < BLOCK_SIZE; j++)
        {
            xor_index = i * BLOCK_SIZE + j;
            pre_xor_index = (i - 1) * BLOCK_SIZE + j;
            d_text[xor_index] = c_text[pre_xor_index] ^ d_text[xor_index];
        }
    }
}
...
void DES_CFB_Enc(BYTE *p_text, BYTE *c_text, BYTE *IV, BYTE *key, int msg_len)
{
    int xor_index, enc_index, pre_enc_index;
    int index = (msg_len % BLOCK_SIZE) ? (msg_len / BLOCK_SIZE + 1) : (msg_len / BLOCK_SIZE);
    int bit = msg_len % BLOCK_SIZE;

    DES_Encryption(IV, c_text, key);

    for (int j = 0; j < BLOCK_SIZE; j++)
    {
        c_text[j] = p_text[j] ^ c_text[j];
    }
    for (int i = 1; i < index; i++)
    {
        enc_index = i * BLOCK_SIZE;
        pre_enc_index = (i - 1) * BLOCK_SIZE;
        DES_Encryption(c_text + pre_enc_index, c_text + enc_index, key);

        for (int j = 0; j < BLOCK_SIZE; j++)
        {
            xor_index = i * BLOCK_SIZE + j;
            c_text[xor_index] = p_text[xor_index] ^ c_text[xor_index];
        }
    }
}
...
void DES_CFB_Dec(BYTE *c_text, BYTE *d_text, BYTE *IV, BYTE *key, int msg_len)
{
    ...
    for (int i = 1; i < index; i++)
    {
        enc_index = i * BLOCK_SIZE;
        pre_enc_index = (i - 1) * BLOCK_SIZE;
        DES_Encryption(c_text + pre_enc_index, d_text + enc_index, key);

        for (int j = 0; j < BLOCK_SIZE; j++)
        {
            xor_index = i * BLOCK_SIZE + j;
            d_text[xor_index] = c_text[xor_index] ^ d_text[xor_index];
        }
    }
}
...
void DES_OFB_Enc(BYTE *p_text, BYTE *c_text, BYTE *IV, BYTE *key, int msg_len)
{
    ...
    for (int j = 0; j < BLOCK_SIZE; j++)
    {
        xor_index = (index - 1) * BLOCK_SIZE + j;
        c_text[xor_index] = p_text[xor_index] ^ c_text[xor_index];
    }
}
void DES_OFB_Dec(BYTE *c_text, BYTE *d_text, BYTE *IV, BYTE *key, int msg_len)
{
    ...
    for (int i = 1; i < index; i++)
    {
        enc_index = i * BLOCK_SIZE;
        pre_enc_index = (i - 1) * BLOCK_SIZE;
        DES_Encryption(d_text + pre_enc_index, d_text + enc_index, key);
...
void DES_CTR_Enc(BYTE *p_text, BYTE *c_text, BYTE *key, UINT64 ctr, int msg_len)
{
    ...
    for (int i = 0; i < index; i++)
    {
        ...
        DES_Encryption(&cvt, c_text + enc_index, key);
        cvt++;
        for (int a = 0; a < index; a++)
        {
            for (int b = 0; b < BLOCK_SIZE; b++)
            {
                // printf("%x ", c_text[a * BLOCK_SIZE + b]);
            }
        }
        // printf("\n");
    }
...
void DES_CTR_Dec(BYTE *c_text, BYTE *d_text, BYTE *key, UINT64 ctr, int msg_len)
{
    ...
    {
        enc_index = i * BLOCK_SIZE;
        DES_Encryption(&cvt, d_text + enc_index, key);
        cvt++;
        for (int a = 0; a < index; a++)
        {
            for (int b = 0; b < BLOCK_SIZE; b++)
            {
                // printf("%x ", d_text[a * BLOCK_SIZE + b]);
            }
        }
        // printf("\n");
    }
