...
def TrainNN(num_hiddens, eps, momentum, num_epochs):
  ...
  for i in range(4):
    W1, W2, b1, b2 = InitNN(inputs_train.shape[0], num_hiddens[i], target_train.shape[0])
  ...
  return W1, W2, b1, b2, train_error_list, valid_error_list
...
def DisplayErrorPlot(train_error, valid_error):
  ...
  plt.clf()
  for i in range(4):
    plt.plot(range(len(train_error[i])), train_error[i], label='Train_'+str(i))
    plt.plot(range(len(valid_error[i])), valid_error[i], label='Validation_'+str(i))
  plt.xlabel('Epochs')
...
def main():
  ...
  eps = 0.02
  momentum = 0.5
  num_epochs = [x*100 for x in num_hiddens]
